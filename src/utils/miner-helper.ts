...
export class MinerHelper {
    ...
    try {
      if (ipAddress != null || ipAddress == 'N/A') {
        ...
        if (data.STATUS != undefined && data.STATUS !== 'E') {
          ...
          if (!raw) return this.parseData(tData);
          return tData;
        }
        // Whatsminer
  ...
  private parseData(data): any {
    ...
    if (isAvalon) {
      ...
      // firmware = data.version[0].VERSION[0].VERSION;
      // software = `CGMiner ${data.version[0].VERSION[0].CGMiner}`;
      // hardware = data.version[0].VERSION[0].HWTYPE;
